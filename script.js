...
let currentTint = ''; // 현재 적용된 틴트 저장 변수
...
video.addEventListener('play', () => {
    function renderFrame() {
        ...
        ctx.drawImage(video, 0, 0, canvas.width, canvas.height); // 비디오 먼저 그리기
        ...
        faceapi.detectAllFaces(video, new faceapi.TinyFaceDetectorOptions())
            ...
            .then(detections => {
                if (detections.length > 0) {
                    ...
                    ctx.clearRect(0, 0, canvas.width, canvas.height); // 비디오만 표시, 틴트 제거
                    currentTint = ''; // 틴트 초기화
        ...
        requestAnimationFrame(renderFrame); // 부드러운 갱신
    }

    renderFrame(); // 첫 번째 프레임 호출
});
